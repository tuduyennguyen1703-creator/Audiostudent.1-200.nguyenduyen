...
<head>
    ...
    <title>Audio From 1 to 200</title>
...
<body class="flex flex-col items-center justify-center p-4">
    ...
    <!-- Start Overlay -->
    <div id="startOverlay" class="fixed inset-0 bg-emerald-900 bg-opacity-95 z-50 flex flex-col items-center justify-center transition-opacity duration-500">
        <div class="text-center p-6 max-w-md">
            <div class="mb-6 text-6xl text-emerald-400 animate-bounce">
                <i class="fas fa-tree"></i>
            </div>
            <h2 class="text-3xl font-bold text-white mb-4">Vocabulary 1-200</h2>
            <p class="text-emerald-200 mb-8">Chủ đề: Môi trường & Đời sống</p>
            <button onclick="startApp()" class="px-8 py-4 bg-emerald-600 text-white text-xl font-bold rounded-2xl shadow-lg hover:bg-emerald-500 hover:scale-105 transition transform">
    ...
    <!-- Header -->
    <header class="w-full max-w-2xl mb-6 bg-white p-4 rounded-xl shadow-lg bg-opacity-90 backdrop-blur-sm border border-emerald-100">
        <div class="flex justify-between items-center mb-3">
            <div>
                <h1 class="text-xl font-bold text-gray-800"><i class="fas fa-leaf text-emerald-600 mr-2"></i>Luyện Nghe Từ Vựng</h1>
                <p class="text-xs text-gray-500 mt-1">
                    <span id="learnedCount" class="text-emerald-600 font-bold">0</span> thuộc / 
                    <span id="learningCount" class="text-red-500 font-bold">0</span> chưa
                </p>
            </div>
            <div class="flex gap-2">
                <button onclick="openListModal()" class="bg-emerald-50 text-emerald-700 px-3 py-2 rounded-lg hover:bg-emerald-100 transition text-sm font-semibold" title="Danh sách từ">
                    <i class="fas fa-list"></i>
                </button>
                <button onclick="document.getElementById('settingsModal').classList.remove('hidden')" class="text-gray-600 hover:text-emerald-600 transition px-2" title="Cài đặt">
        ...
        <!-- Mode Toggle -->
        <div class="flex p-1 bg-emerald-50 rounded-lg">
        ...
        <div id="flashcard" class="flip-card-inner w-full h-full relative">
            ...
            <div class="card-front bg-white rounded-3xl shadow-2xl flex flex-col items-center justify-center p-8 border-2 border-white relative">
                ...
                <div class="mb-8 relative">
                    <div class="absolute -inset-4 bg-emerald-100 rounded-full animate-pulse opacity-50"></div>
                    <button onclick="playAudio()" class="relative w-32 h-32 bg-emerald-600 rounded-full flex items-center justify-center text-white shadow-xl hover:bg-emerald-700 hover:scale-105 transition transform duration-200 group">
                        <i class="fas fa-volume-up text-5xl group-hover:animate-bounce"></i>
                    </button>
                </div>
                
                <p class="text-gray-500 text-center mb-6 text-lg">Nhấn loa để nghe, sau đó đoán từ và nghĩa.</p>

                <button onclick="revealCard()" class="px-8 py-3 bg-white border-2 border-emerald-600 text-emerald-600 font-bold rounded-full hover:bg-emerald-50 transition shadow-md">
            ...
            <div class="card-back bg-white rounded-3xl shadow-2xl flex flex-col items-center justify-center p-8 border-2 border-white">
                ...
                <div class="absolute top-6 right-6">
                    <button onclick="playAudio()" class="w-10 h-10 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 hover:bg-emerald-200 transition">
                ...
                <div class="text-center w-full flex-grow flex flex-col justify-center mt-4">
                    ...
                    <p id="ipaText" class="text-xl text-emerald-500 font-mono mb-4">/seɪf/</p>
                    <div class="w-16 h-1 bg-emerald-200 mx-auto mb-4 rounded"></div>
                ...
                <div class="w-full grid grid-cols-2 gap-3 mb-4">
                    ...
                    <button onclick="markWord(true)" id="btnLearned" class="py-2 rounded-lg border border-emerald-200 text-emerald-600 hover:bg-emerald-50 font-medium transition flex items-center justify-center">
                ...
                <div class="w-full flex gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button onclick="nextCard()" class="flex-1 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition shadow-lg flex items-center justify-center">
    ...
    <div class="w-full max-w-2xl mt-8 flex justify-between items-center px-4">
        <button onclick="prevCard()" class="w-12 h-12 rounded-full bg-white shadow text-gray-600 hover:text-emerald-600 flex items-center justify-center transition">
        ...
        <div class="flex items-center gap-4">
            ...
            <button onclick="toggleShuffle()" id="shuffleBtn" class="w-10 h-10 rounded-full bg-white shadow text-gray-400 flex items-center justify-center transition hover:text-emerald-600" title="Xáo trộn từ">
                <i class="fas fa-random"></i>
            </button>
        </div>

        <button onclick="nextCard()" class="w-12 h-12 rounded-full bg-white shadow text-gray-600 hover:text-emerald-600 flex items-center justify-center transition">
    ...
    <div id="settingsModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl transform transition-all scale-100">
            ...
            <div class="mb-4">
                ...
                <p class="text-xs text-gray-500 mt-2 italic">Mẹo: Sử dụng trình duyệt Microsoft Edge để có giọng đọc "Natural" tự nhiên nhất.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Tốc độ đọc: <span id="rateValue">1.0</span>x</label>
                <input type="range" id="rateRange" min="0.5" max="1.5" step="0.1" value="1" class="w-full h-2 bg-emerald-200 rounded-lg accent-emerald-600">
            </div>
            <div class="mb-4 flex items-center justify-between">
                <label class="text-sm font-medium text-gray-700">Tự động phát khi chuyển bài</label>
                <input type="checkbox" id="autoPlay" class="w-5 h-5 text-emerald-600 rounded" checked>
            </div>
            ...
            <button onclick="document.getElementById('settingsModal').classList.add('hidden')" class="w-full py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700">Đóng</button>
        </div>
    </div>

<script>
    // DATA SOURCE: 200 WORDS WITH PHONETICS
    ...
    function loadProgress() {
        // Unique storage key for this 200-word list
    ...
    function loadVoices() {
        ...
        // Sort: Prioritize "Natural", "Google", "Premium"
        englishVoices.sort((a, b) => {
            const isHighQuality = (name) => name.includes('Google') || name.includes('Natural') || name.includes('Premium') || name.includes('Enhanced');
            if (isHighQuality(a.name) && !isHighQuality(b.name)) return -1;
            if (!isHighQuality(a.name) && isHighQuality(b.name)) return 1;
            return 0;
        });
        ...
        // Smart Select: Try to find specific good voices
        ...
        // Find best match or default to first in sorted list
        const bestVoice = englishVoices.find(v => preferredVoices.some(p => v.name.includes(p))) || englishVoices[0];
    ...
    voiceSelect.addEventListener('change', () => { if(hasStarted) speak("Hello."); });
    ...
    function resetAudioEngine() {
        ...
        window.speechSynthesisUtterances = []; 
        loadVoices();
        alert("Đã reset bộ phát âm thanh.");
    }
    ...
    function speak(text) {
        ...
        window.speechSynthesisUtterances.push(utterance);
        ...
        utterance.pitch = 1;
        utterance.onend = () => { window.speechSynthesisUtterances = window.speechSynthesisUtterances.filter(u => u !== utterance); };
        utterance.onerror = (e) => { if(e.error !== 'interrupted') console.error(e); };
    ...
    function toggleShuffle() {
        ...
        if (isShuffled) {
            shuffleBtn.classList.add('text-emerald-600', 'bg-emerald-50');
            ...
            shuffleBtn.classList.remove('text-emerald-600', 'bg-emerald-50');
    ...
    function renderList() {
        ...
        filteredList.forEach(item => {
            ...
            div.className = "bg-white p-4 rounded-xl border border-emerald-100 mb-3 flex justify-between items-center shadow-sm hover:shadow-md transition";
            div.innerHTML = `
                ...
                <div class="flex gap-2 flex-shrink-0">
                     <button onclick="playListAudio('${item.en.replace(/'/g, "\\'")}')" class="w-10 h-10 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center hover:bg-emerald-100 transition">
                        <i class="fas fa-volume-up"></i>
                    </button>
                    ${targetState ? 
